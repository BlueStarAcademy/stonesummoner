...
**「상징으로 몬스터를 키우고, 소환사가 마법진에서 마나를 모아 전장을 뒤집는다.」**
...
2. **소환사가 직접 전투 참여** + **소환사 장비** 장착/강화
3. **마나**가 차면 소환사 스킬 — 충전 속도가 **마법진 국면에 연동**
...
```mermaid
flowchart LR
  Home[홈섬_건물_수집] --> Build[상징_각성_소환사장비]
...
2. 소환·상징·소환사 장비로 스펙 확보  
3. 전투: 스톤소환 → Amplify + 마나 가속 → 몬스터/소환사 스킬  
4. 보상·영광포인트로 다시 홈 성장  
...
- 승패: **적 소환수 전멸 = 승리** / **아군 소환수 전멸 = 패배** (소환사는 후열·비대상)
- 집 계산 없음
...
| 몬스터 패밀리 **50 × 속성 5 (250)** + 상징 세트 | Phase 2 로스터 · 속성별 틴트 초상 |
| 소환사 + 장비·세트5·가방 + 각성 + 마나 스킬 5 + 스킬 트리 15 + 장비 금고 | (풀 스킬 트리·세트 심화) |
| 전투: **5×5→7×7→9×9** 착수·따냄·Amplify·마나 · 9×9 50수 리셋 | 모듈 B~H · 13×13 |
| 보드 아이템 모듈 A 8종 (변환가루 희귀) | 형상·이벤트·보스 보드 확장 |
