...
- 몬스터 스킬 피해 계수는 **ATK% 그대로** 저장한다 (예: S1 공격형 `3.7` = 370%).
- **태생 성급은 스킬 %를 올리지 않는다.** 1★~5★ 동일 역할은 같은 S1/S2/S3 %를 쓴다.
- 태생 차이는 **기본 스탯 곡선** + 완만한 진화 배율로 유지한다 (6★ 만렙에서도 5★가 1★보다 분명히 강함).
- 스킬 레벨업 피해 성장은 레벨당 **+5%** (`SKILL_LEVEL_POWER_PCT`, SW 스킬업에 가깝게).
- 진화는 스탯만 올리고 스킬 계수는 올리지 않는다.
