    ...
    switch (def.kind) {
      ...
      case "single_damage": {
        ...
        if (target) {
          ...
          const perHit = power / hits;
          for (let hit = 0; hit < hits; hit += 1) {
            results.push(this.applyHit(unit, target, perHit, true));
            if (def.ailment) this.applyMagicAilment(unit, target, def.ailment);
          }
        }
        break;
      }
      case "enemy_ailment": {
        ...
        const picked =
          def.hitCount && def.hitCount > 0
            ? foes
                .slice()
                .sort((a, b) => a.hp / a.stats.hp - b.hp / b.stats.hp)
                .slice(0, def.hitCount)
            : foes;
      ...
      case "ally_cleanse": {
        const allies = aliveSummons(this.units, unit.team);
        const picked =
          def.hitCount && def.hitCount > 0
            ? allies
                .slice()
